...
export async function createBooking(booking: InsertBooking): Promise<Booking | null> {
  ...
  try {
    ...
    // Get the last inserted booking by ordering by ID descending
    const newBooking = await db.select().from(bookings).orderBy((t) => t.id).limit(1);
    
    return newBooking.length > 0 ? newBooking[0] : null;
  } catch (error) {
